...
import {
  ...
  UserCheck, UserX, DollarSign, UserPlus, Cloud, MapPin, ClipboardEdit
} from "lucide-react";
...
// URL de base toujours absolue — jamais de chemin relatif
const BASE_URL = "https://btp-gestion-de-projet.onrender.com";
...
function apiFetch(path: string, opts?: RequestInit) {
  ...
  return fetch(fullUrl, {
    ...
    if (!r.ok) { const d = await r.json().catch(() => ({})); throw new Error(d?.message || `Erreur ${r.status}`); }
    return r.json();
  });
}
...
export default function PointageNew() {
  ...
  // Modal ajout d'ouvrier à la volée
  ...
  useEffect(() => {
    ...
    // Appels séparés pour isoler les erreurs
    const loadWorkers = apiFetch(`/api/pointage/workers-for-project/${projectId}`)
      .then(data => Array.isArray(data) ? data : [])
      .catch(() => []);

    const loadTasks = apiFetch(`/api/projects/${projectId}/tasks`)
      .then(data => Array.isArray(data) ? data : [])
      .catch(() => []); // tâches non critiques

    Promise.all([loadWorkers, loadTasks]).then(([workerData, taskData]) => {
      setTasks(taskData);

      if (workerData.length === 0) {
        toast({ title: "Aucun ouvrier trouvé", description: "Ajoutez du personnel dans la section Personnel.", variant: "destructive" });
        setPersonnel([]);
        setWorkers([]);
        return;
      }
  ...
  // Ajout d'un ouvrier à la volée — création immédiate côté backend pour
  // garantir qu'il apparaisse dans la liste du personnel
  const handleAddWorkerInline = async (data: WorkerData) => {
    ...
    try {
      ...
      // Ajoute la nouvelle ouvrier à la fois dans personnel local ET dans workers
      const newP: PersonnelRecord = {
        id: created.id,
        name: created.name,
        trade: created.trade || "",
      ...
      toast({ title: "Ouvrier ajouté", description: `${data.name} est dans la fiche et dans le personnel.` });
    } catch (err: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const result = await apiFetch("/api/pointage", {
        method: "POST",
        body: JSON.stringify({
          projectId: parseInt(projectId), date,
      ...
      toast({ title: "Erreur lors de la création", description: err.message, variant: "destructive" });
    ...
    <AppLayout title="Nouveau Pointage">
      <div className="max-w-4xl mx-auto space-y-5 pb-24">
        ...
        <div className="bg-white rounded-2xl border border-border/50 shadow-sm p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Select value={projectId} onValueChange={setProjectId}>
                <SelectTrigger className="rounded-xl h-10"><SelectValue placeholder="Sélectionner un chantier..." /></SelectTrigger>
                <SelectContent>
          ...
          {/* Champs pro enrichis */}
          <div className="mt-4 pt-4 border-t border-border/40 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="space-y-1.5">
              <Label className="text-xs flex items-center gap-1"><Cloud className="w-3 h-3" /> Météo du jour</Label>
            ...
            <div className="space-y-1.5">
              <Label className="text-xs flex items-center gap-1"><MapPin className="w-3 h-3" /> Zone du chantier</Label>
              <Input
                value={siteLocation}
            ...
            <div className="space-y-1.5 sm:col-span-2 lg:col-span-4">
              ...
              <Textarea
                ...
                placeholder="Incidents, livraisons matériaux, points de sécurité…"
                rows={2}
          ...
          <div className="bg-white rounded-2xl border border-border/50 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-border/20 flex flex-col sm:flex-row sm:items-center gap-2">
              <h2 className="text-base font-bold flex items-center gap-2 flex-shrink-0">
                ...
                Présences <span className="text-sm font-normal text-muted-foreground">({workers.length})</span>
              </h2>
              ...
              <div className="flex gap-1.5 flex-shrink-0">
                ...
                <button onClick={() => setIsAddWorkerOpen(true)}
                  className="flex items-center gap-1 px-2.5 py-1.5 text-[11px] font-semibold rounded-lg bg-secondary/10 text-secondary border border-secondary/30 hover:bg-secondary/20 transition-colors">
                  <UserPlus className="w-3 h-3" /> Nouvel ouvrier
                </button>
              </div>
            </div>
            ...
            <div className="divide-y divide-border/15">
              ...
              {filteredWorkers.map(w => {
                ...
                return (
                  <WorkerRow key={w.personnelId} worker={w} tasks={tasks}
                  ...
                  {workers.map(w => {
                    ...
                    return (
                      <tr key={w.personnelId} className={w.status === "ABSENT" ? "opacity-50" : ""}>
                        <td className="px-4 py-2.5 font-medium text-sm">{w.name}{w.trade && <span className="text-xs text-muted-foreground ml-1.5">{w.trade}</span>}</td>
                        <td className="px-3 py-2.5 text-center"><span className={`px-2 py-0.5 rounded-full text-xs font-semibold border ${STATUS_PILL[w.status]}`}>{STATUS_LABEL[w.status]}</span></td>
        ...
        <WorkerModal
          ...
          onSubmit={handleAddWorkerInline}
          isLoading={isAddingWorker}
          title="Nouvel ouvrier (créé et ajouté à la fiche)"
          submitLabel="Créer et ajouter au pointage"
        />
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur border-t border-border/50 p-4 shadow-lg z-20">
            <div className="flex items-center justify-between gap-4 max-w-4xl mx-auto">
              ...
              <div className="flex gap-3">
                ...
                <Button onClick={handleSubmit} disabled={isSaving || !projectId} className="rounded-xl bg-secondary hover:bg-secondary/90 text-white shadow-lg px-6">
  ...
  return (
    <div className={w.status === "ABSENT" ? "bg-red-50/30" : w.status === "DEMI_JOURNEE" ? "bg-yellow-50/20" : ""}>
      <div className="flex items-center gap-3 px-4 py-3">
        <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${w.status === "ABSENT" ? "bg-red-100 text-red-600" : w.status === "DEMI_JOURNEE" ? "bg-yellow-100 text-yellow-700" : "bg-primary/10 text-primary"}`}>{initials}</div>
        ...
        <div className="flex gap-1 flex-shrink-0">
          {(["PRESENT", "DEMI_JOURNEE", "ABSENT"] as const).map(s => (
            <button key={s} onClick={() => onChange("status", s)}
              className={`w-8 h-7 rounded-lg text-xs font-bold border transition-all ${w.status === s ? STATUS_PILL[s] + " shadow-sm" : "bg-transparent text-muted-foreground border-border/40 hover:bg-muted/30"}`}>
              {s === "PRESENT" ? "✓" : s === "ABSENT" ? "✗" : "½"}
            </button>
          ))}
        </div>
      ...
      {w.expanded && (
        <div className="px-4 pb-5 pt-2 border-t border-border/15 bg-muted/5 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <div>
                {w.payMode === "PAR_JOUR" ? (
                  ...
                  <Input type="number" value={w.dailyWage || ""} onChange={e => onChange("dailyWage", parseFloat(e.target.value) || 0)} placeholder="10000" className="h-9 rounded-xl mt-1 text-sm" /></>
                ) : (
                  <><Label className="text-xs">Montant tâche (FCFA)</Label>
                  <Input type="number" value={w.taskAmount || ""} onChange={e => onChange("taskAmount", parseFloat(e.target.value) || 0)} placeholder="50000" className="h-9 rounded-xl mt-1 text-sm" /></>
                )}
              </div>
            </div>
          )}
          ...
          {w.status !== "ABSENT" && w.payMode === "PAR_TACHE" && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input type="range" min={0} max={100} step={5} value={w.taskProgressPct} onChange={e => onChange("taskProgressPct", parseInt(e.target.value))} className="w-full mt-2.5 accent-primary" />
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <Textarea value={w.notes} onChange={e => onChange("notes", e.target.value)} placeholder="Remarques..." rows={2} className="rounded-xl mt-1 resize-none text-sm" />
          </div>
        </div>
      )}
    </div>
