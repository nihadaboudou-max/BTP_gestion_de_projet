...
function ChefAdminList({ onNew }: { onNew: () => void }) {
  ...
  const { data: activeSheets, isLoading: loadingActive } = useListPointageSheets();
  const { data: archivedSheets, isLoading: loadingArchived } = useQuery({
    queryKey: ["/api/pointage", "archived"],
    queryFn: () => {
      const token = localStorage.getItem("hairou_token");
      return fetch(`${BACKEND}/api/pointage?onlyArchived=true`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      }).then(r => r.json());
    },
  });

  const sheets = tab === "active" ? activeSheets : archivedSheets;
  const isLoading = tab === "active" ? loadingActive : loadingArchived;
  const activeCount = (activeSheets as any[])?.length ?? 0;
  const archivedCount = (archivedSheets as any[])?.length ?? 0;
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-1 bg-muted/30 p-1 rounded-xl w-fit">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${tab === "active" ? "bg-white shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"}`}
          >
            <Folder className="w-3.5 h-3.5 inline mr-1.5" />
            Actives
            <span className="ml-2 text-xs bg-muted rounded-full px-1.5 py-0.5">{activeCount}</span>
          </button>
          <button
            onClick={() => setTab("archived")}
            className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${tab === "archived" ? "bg-white shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"}`}
          >
            <Archive className="w-3.5 h-3.5 inline mr-1.5" />
            Archivées
            <span className="ml-2 text-xs bg-muted rounded-full px-1.5 py-0.5">{archivedCount}</span>
          </button>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center p-16 text-center bg-white rounded-3xl border border-border/50 shadow-sm">
            <div className="w-20 h-20 bg-muted rounded-full flex items-center justify-center mb-4">
              {tab === "archived" ? <Archive className="w-10 h-10 text-muted-foreground/50" /> : <Clock className="w-10 h-10 text-muted-foreground/50" />}
            </div>
            <h3 className="text-xl font-bold mb-2">{tab === "archived" ? "Aucune fiche archivée" : "Aucune fiche de pointage"}</h3>
            <p className="text-muted-foreground mb-6">
              {tab === "archived" ? "Les fiches archivées apparaîtront ici." : "Commencez par créer une feuille de présence pour un chantier."}
            </p>
            {tab === "active" && <Button onClick={onNew} className="rounded-xl">Nouveau Pointage</Button>}
          </div>
        ) : (
