...
// Returns ALL active personnel + ALL approved users (any role) so that
// chef de chantier and other staff can also be pointed.
router.get("/workers-for-project/:projectId", authenticate, async (req: AuthRequest, res) => {
  try {
    const projectId = parseInt(req.params.projectId);

    // 1. Personnel records (ouvriers, etc.) assigned to the project
    ...
    // 2. All active personnel as fallback (when none are explicitly assigned)
    const allPersonnel = assigned.length > 0 ? assigned : await db
      .select({
        id: personnelTable.id,
        name: personnelTable.name,
        trade: personnelTable.trade,
    ...
    // 3. All approved users (any role: ADMIN, CHEF_CHANTIER, OUVRIER, etc.)
    //    who are NOT already in the personnel list (avoid duplicates by name)
    const allUsers = await db
      .select({
        id: sql<number>`id * -1`, // negative IDs to distinguish from personnel IDs
        name: usersTable.name,
        trade: sql<string>`role`,   // use role as trade label
        dailyWage: sql<string>`'0'`,
        isActive: usersTable.isActive,
        assignedToProject: sql<boolean>`false`,
        source: sql<string>`'user'`,
        userId: usersTable.id,
      })
      .from(usersTable)
      .where(eq(usersTable.isActive, true));

    // Merge: users not already in personnel list (by name, case-insensitive)
    const personnelNames = new Set(allPersonnel.map((p: any) => p.name.toLowerCase()));
    const userWorkers = allUsers
      .filter((u: any) => !personnelNames.has(u.name.toLowerCase()))
      .map((u: any) => ({
        ...u,
        trade: u.trade === "CHEF_CHANTIER" ? "Chef de Chantier"
             : u.trade === "ADMIN"         ? "Administrateur"
             : u.trade === "OUVRIER"       ? "Ouvrier"
             : u.trade,
      }));
