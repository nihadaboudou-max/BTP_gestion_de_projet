...
router.get("/", authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    if (role === "OUVRIER") baseFilters.push(eq(projectsTable.chefId, req.user!.userId));
    if (onlyArchived) baseFilters.push(sql`${projectsTable.archived} = 1`);
    else if (!includeArchived) baseFilters.push(sql`${projectsTable.archived} = 0`);

    const where = baseFilters.length > 0
      ? baseFilters.reduce((acc, f) => acc ? sql`${acc} AND ${f}` : f, null as any)
      : null;

    let projects;
    if (where) {
      projects = await db.select().from(projectsTable).where(where).orderBy(projectsTable.createdAt);
    } else {
...
router.post("/:id/archive", authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const [project] = await db.update(projectsTable)
      .set({ archived: 1, archivedAt: new Date(), updatedAt: new Date() })
    ...
    req.log.error({ err }, "Archive project error");
...
router.post("/:id/unarchive", authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const id = parseInt(req.params.id);
    const [project] = await db.update(projectsTable)
      .set({ archived: 0, archivedAt: null, updatedAt: new Date() })
      .where(eq(projectsTable.id, id))
      .returning();
    if (!project) {
      res.status(404).json({ error: "Non trouvé" });
    ...
    req.log.error({ err }, "Unarchive project error");
    res.status(500).json({ error: "Erreur serveur" });
  }
});
